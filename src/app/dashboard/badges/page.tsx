...
export default async function BadgesPage() {
  ...
  const badges = [
    ...
    },
    {
      ...
      iconElement: <Star className="w-6 h-6" />,
      earned: streak >= 7,
    ...
    },
    {
      ...
      iconElement: <GraduationCap className="w-6 h-6" />,
      earned: allPlans.some((p) => {
        const total = p.problems.length;
        const solved = p.problems.filter((pp) => pp.status === "SOLVED").length;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {badges.map((badge) => {
          ...
          return (
            ...
            >
              <div className="mb-3" style={{ color: badge.earned ? "var(--accent-text)" : "var(--text-muted)" }}>{badge.iconElement}</div>
