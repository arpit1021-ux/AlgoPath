  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2">
      {toasts.map((toast) => (
        ...
        >
          {toast.type === "success" ? <CheckCircle2 className="w-4 h-4 inline mr-1.5" /> : toast.type === "error" ? <XCircle className="w-4 h-4 inline mr-1.5" /> : <Info className="w-4 h-4 inline mr-1.5" />}
          {toast.message}
        </div>
      ))}
    </div>
